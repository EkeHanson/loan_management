...
const ComplianceCheckTable = ({ complianceChecklist = [], jobApplicationId }) => {
  ...
  const initialComplianceData = jobApplicationId?.job_application?.compliance_status?.length > 0
    ? jobApplicationId.job_application.compliance_status.map(item => ({
        id: item.id,
        title: item.name || 'Unnamed Requirement', // Fallback in case name is missing
  ...
  // Initialize isSubmitted based on whether any compliance status is 'uploaded' or 'In Review'
  const hasSubmittedDocuments = jobApplicationId?.job_application?.compliance_status?.some(
  ...
  const [isSubmitted, setIsSubmitted] = useState(hasSubmittedDocuments);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showRejectionModal, setShowRejectionModal] = useState(false);
  const [rejectionReason, setRejectionReason] = useState('');
  const fileInputRefs = useRef(complianceData.map(() => null));
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      const compliance_checklist = jobApplicationId?.job_application?.job_requisition?.compliance_checklist || [];
      const complianceMap = new Map(compliance_checklist.map(item => [item.id, item.name]));

      complianceData.forEach((item, index) => {
        if (item.file) {
          formData.append(`documents`, item.file);
          formData.append(`document_ids`, item.id); // Send compliance item ID
          formData.append(`document_names`, complianceMap.get(item.id) || item.title); // Send compliance item name
        }
      });
      ...
      const response = await axios.put(
        `${config.API_BASE_URL}/api/talent-engine-job-applications/document/applications/applicant/upload/${jobApplicationId.job_application.id}/compliance-update/`,
        formData,
        {
          headers: {
            // Remove manual Content-Type to let axios handle it
          },
        }
      );
      ...
      const updatedData = job_application.compliance_status.map((item) => ({
        id: item.id,
        title: item.name || complianceMap.get(item.id) || 'Unnamed Requirement',
        fileName: item.document?.file_url?.split('/').pop() || '',
        fileType: getFileTypeInfo(item.document?.file_url || '').type,
        fileIcon: getFileTypeInfo(item.document?.file_url || '').icon,
        fileUrl: item.document?.file_url || '',
      ...
      showAlert(error.message || 'Failed to submit for review. Please try again.', 'error');
      setIsSubmitting(false);
    }
  };
          ...
          {complianceData.map((item, index) => {
            ...
            return (
              <tr key={item.id}>
                ...
                <td>{jobApplicationId.job_application.full_name} {item.title || <span className="no-file">No file</span>}</td>
