...
const confirmSchedule = async () => {
  if (!tempStartTime) {
    setErrorMessage('Please select a start time for the interview.');
    setTimeout(() => setErrorMessage(''), 5000); // Increase timeout
    return;
  }
  ...
  try {
    ...
    const suggestion = error.suggestion || 'Please try again or contact support if the issue persists.';
    setErrorMessage(`${errorMessage} ${suggestion}`);
    setIsLoading(false);
    setTimeout(() => setErrorMessage(''), 8000); // Increase timeout to 8 seconds for visibility
    ...
    <div className="Schedule-MMAin-Pais">
      <AnimatePresence>
        {errorMessage && (
          ...
          >
            <svg viewBox="0 0 24 24" style={{ width: '24px', height: '24px', marginRight: '0.75rem', fill: '#721c24' }}>
