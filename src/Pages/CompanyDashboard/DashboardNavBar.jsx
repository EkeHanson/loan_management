...
import { useMobileNav } from '../../context/MobileNavContext'; // ✅ Import the context
...
const DashboardNavBar = () => {
  ...
  return (
    <div className="DashboardNavBar">
      ...
      <button
        ref={buttonRef}
        className="mobile-Show genn-Drop-Togler"
        onClick={() => setMobileNavActive(prev => !prev)}
      >
        {mobileNavActive ? (
          // ❌ Close Icon SVG (new)
          <svg
            className={`nav-toggle-icon ${mobileNavActive ? 'rotate-icon' : ''}`}
            width="27"
            height="27"
            viewBox="0 0 27 27"
            fill="none"
          ...
          >
            <path
              d="M24.9811 0L26.3386 1.35744L1.35744 26.3366L0 24.9802L24.9811 0Z"
              fill="#fff"
            />
            <path
              d="M1.35744 0L26.3386 24.9792L24.9811 26.3376L0 1.3584L1.35744 0Z"
              fill="#fff"
            />
          </svg>
        ) : (
          ...
          <button
            ref={buttonRef}
            className={`mobile-Hide genn-Drop-Togler ${showFeaturesDropdown ? 'active-Gent-Trangl' : ''}`}
        ...
        <div className="NaV-2">
          <div className="NaV-2-Icons">
             ...
             <span
              ref={buttonRef}
              className='daf-Svggol mobile-Show'
              title="Features Launcher"
              onClick={() => setShowFeaturesDropdown(!showFeaturesDropdown)}
            >
                <svg viewBox="0 0 49 49" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <Link to="/company" title="Dashboard">
            ...
            <span
              ref={chatRef}
              title="Chat"
              className={`cursor-pointer mobile-Hide ${activeButton === 'chat' ? 'active' : ''}`}
            ...
            <span
              ref={calendarRef}
              title="Calendar"
              className={`cursor-pointer mobile-Hide ${activeButton === 'calendar' ? 'active' : ''}`}
              onClick={() => handleButtonClick('calendar')}
            >
              <CalendarIconOutline className="h-6 w-6" />
            </span>

            <Link to="/company/notifications" title="Notifications" className='mobile-Hide'>
              {notificationCount > 0 && <i className="nottti-Inddi GenNot"></i>}
              <BellIconOutline className="h-6 w-6" />
            </Link>

            <Link to="/company/settings" title="Settings" className='mobile-Hide'>
              ...
              {showProfileDropdown && (
                ...
                >
                  <div className="All-TTo-Nagbs-main ouj-pia">
                   ...
                   <Link to="/company/notifications" className='mobile-Show' onClick={closeProfileDropdown}>
                    <BellIconOutline /> Notifications
                  </Link>
                   <Link to="/company/settings" className='mobile-Show' onClick={closeProfileDropdown}>
